    ...
    <div className="space-y-8 max-w-6xl">
        ...
        {(
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${
              filter === key ? "bg-primary text-white" : "text-neutral-600 hover:bg-neutral-100 dark:text-neutral-300 dark:hover:bg-neutral-800"
            }`}
          >
            {label}
          </button>
      ...
      : <div className="space-y-4">
          {users.map((u) => (
            <div
              key={u.id}
              className="bg-white rounded-lg border border-neutral-200 shadow-xs p-5 flex flex-col lg:flex-row gap-6 lg:items-start dark:border-neutral-700 dark:bg-neutral-900"
            >
              <div className="flex gap-3 shrink-0">
                ...
                />
                <div>
                  <div className="font-medium text-base text-neutral-900 dark:text-neutral-100">{u.name}</div>
              ...
              <div className="flex-1 space-y-2 text-sm text-neutral-700 min-w-0 dark:text-neutral-300">
                <div className="flex items-start gap-2">
                  <Mail size={16} className="text-neutral-400 shrink-0 mt-0.5 dark:text-neutral-500" />
                  <span className="break-all">{u.email}</span>
                </div>
                <div className="flex items-start gap-2">
                  <Building2 size={16} className="text-neutral-400 shrink-0 mt-0.5 dark:text-neutral-500" />
                  <span>
                    {u.facultyName}
                ...
                {u.position ?
                  <div className="flex items-start gap-2">
                    <Briefcase size={16} className="text-neutral-400 shrink-0 mt-0.5 dark:text-neutral-500" />
                    <span>{u.position}</span>
                  </div>
                : null}
                {u.employeeId ?
                  <div className="flex items-start gap-2">
                    <CreditCard size={16} className="text-neutral-400 shrink-0 mt-0.5 dark:text-neutral-500" />
                    <span>{u.employeeId}</span>
                  </div>
                : null}
                {u.phone ?
                  <div className="flex items-start gap-2">
                    <Phone size={16} className="text-neutral-400 shrink-0 mt-0.5 dark:text-neutral-500" />
                    <span>{u.phone}</span>
                  </div>
                : null}
              ...
              <div className="flex flex-col gap-2 items-stretch lg:items-end shrink-0 min-w-[180px]">
                ...
                {u.approvalStatus === "APPROVED" && (
                  <div className="text-right space-y-1">
                    ...
                    {u.approverName ?
                      <div className="text-xs text-neutral-500 dark:text-neutral-400">Бекіткен: {u.approverName}</div>
                    : null}
                    {u.approvedAt ?
                ...
                {u.approvalStatus === "REJECTED" && (
                  <div className="text-right space-y-1 max-w-xs">
                    ...
                    {u.approvalNote ?
                      <p className="text-xs text-neutral-600 text-left lg:text-right dark:text-neutral-400">{u.approvalNote}</p>
                    : null}
                  </div>
                )}
              </div>
      ...
      {confirmApprove && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <div className="bg-white rounded-lg shadow-lg max-w-md w-full p-6 space-y-4 dark:bg-neutral-900">
      ...
      {rejectRow && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <div className="bg-white rounded-lg shadow-lg max-w-md w-full p-6 space-y-4 dark:bg-neutral-900">
            ...
            <textarea
              className="w-full min-h-[100px] rounded-md border border-neutral-200 px-3 py-2 text-sm dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-100"
